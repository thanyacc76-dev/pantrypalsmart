...
# Page Configuration
st.set_page_config(
    page_title="PantryPalSmart | AI Food Storage", page_icon="🥑", layout="centered"
)
...
if uploaded_file is not None:
    ...
    if st.button("Inspect Item", type="primary", use_container_width=True):
        with st.spinner("Analyzing item with AI models..."):
            try:
                ...
                detected_food = assets["food_classes"][np.argmax(food_preds[0])]
                ...
                # RandomForest 5-Feature Prediction
                if "food_encoder" in assets and "status_encoder" in assets:
                    ...
                    storage_enc = (
                        assets["storage_encoder"].transform(["refrigerator"])[
                            0
                        ]
                    ...
                    features = np.array([[food_enc, status_enc, storage_enc, 4.0, 70.0]])
