...
# Custom CSS: Light Pastel Theme, Forced Light Inputs, Header Styling, & Falling Animations
st.markdown(
    """
    <style>
    /* Force Light Styling Across System Themes */
    :root {
        color-scheme: light !important;
    }

    /* Base Background and Breathing Gradient */
    ...
    /* FIX ISSUE #2: Streamlit Top Header Bar Theme Matching */
    ...
    /* FIX ISSUE #4: Forced Input Visibility Regardless of Dark Mode */
    label, p, span, div, h1, h2, h3, h4, h5, h6 {
        color: #4a041f !important;
    }
    
    /* Input Boxes, Dropdowns, and Number Selectors Styling */
    ...
    /* FIX ISSUE #1: Continuous Falling Food Animation */
    .falling-container {
        position: fixed;
...
if st.session_state.page == "landing":
    # Continuous Food Falling Overlay on Landing
    st.markdown(
        """
        <div class="falling-container">
            <div class="food-item" style="left: 5%; animation-delay: 0s;">🍒</div>
            <div class="food-item" style="left: 14%; animation-delay: 1.2s;">🍊</div>
            <div class="food-item" style="left: 23%; animation-delay: 0.5s;">🍗</div>
            <div class="food-item" style="left: 32%; animation-delay: 2.1s;">🥚</div>
            <div class="food-item" style="left: 41%; animation-delay: 0.8s;">🥛</div>
            <div class="food-item" style="left: 50%; animation-delay: 1.7s;">🥦</div>
            <div class="food-item" style="left: 59%; animation-delay: 0.3s;">🍎</div>
            <div class="food-item" style="left: 68%; animation-delay: 2.5s;">🍇</div>
            <div class="food-item" style="left: 77%; animation-delay: 1.0s;">🥕</div>
            <div class="food-item" style="left: 86%; animation-delay: 1.9s;">🥒</div>
            <div class="food-item" style="left: 93%; animation-delay: 0.6s;">🍓</div>
        </div>
    """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        <div class="glass-card" style="text-align: center; margin-bottom: 25px;">
            <h2 style="font-size: 1.8rem; font-weight: 800; color: #881337 !important; margin-bottom: 6px;">
                Hey, what are we cooking today? 🍳
            </h2>
            <p style="color: #be123c !important; font-size: 1rem; margin: 0;">
...
# ================= PAGE 2: MAIN APP =================
    ...
    st.markdown("### Step 1: Upload or Take a Photo of Ingredient")
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            if "fresh" in fresh_status.lower():
                is_fresh = True
    ...
    # Step 2: Shelf Life Estimator
    st.markdown("## Step 2: Check freshness & storage duration")
    ...
    storage_type = st.selectbox("Storage type", ["fridge", "pantry", "freezer"])
    temp_val = st.slider(
        "Temperature (°C)",
        min_value=-10.0,
        max_value=40.0,
    ...
    if st.button("Predict remaining freshness"):
        try:
            food_enc = (
            ...
            feat_arr = np.array([[food_enc, 0, storage_enc, temp_val, humidity_val]])
            est_days = assets["expiry_model"].predict(feat_arr)[0]

            st.markdown(
                f"""
                <div class="glass-card">
                    <h3 style="color:#881337 !important; margin:0;">Estimated Remaining Freshness</h3>
                    <p style="font-size:2rem; font-weight:800; color:#be123c !important; margin:5px 0 0 0;">
    ...
    # FIX ISSUE #3: RECIPES ARE ONLY SHOWN WHEN FOOD IS IDENTIFIED & FRESH
    if assessment_done and is_fresh:
        st.markdown("---")
        st.markdown(
            f"### 🎉 Your {selected_food} is fresh! Here are some recipe ideas:"
        )

        with st.expander(f"🍳 Savory Shrimps & {selected_food.title()} Skillet Recipe (⭐ 5.0)"):
            st.write(
                f"Sauté fresh {selected_food} slices along with deveined shrimps in butter, garlic, and fresh herbs."
            )
            st.caption("Prep: 15 M | Cook: 10 M")

        with st.expander(
            f"🍳 Fresh {selected_food.title()} Tart / Galette Recipe (⭐ 5.0)"
        ):
            st.write(
                f"A delicious bakery-style dessert made with fresh sliced {selected_food}, warm cinnamon, and a buttery pastry crust."
            )
            st.caption("Prep: 20 M | Cook: 25 M")

        with st.expander(
            f"🍳 Spiced {selected_food.title()} Garden Salad Recipe (⭐ 5.0)"
        ):
            st.write(
                f"Toss sliced {selected_food} with fresh greens, walnuts, and a light citrus vinaigrette."
            )
            st.caption("Prep: 10 M | Cook: 0 M")
    elif assessment_done and not is_fresh:
        st.warning(
            f"⚠️ The uploaded {selected_food} does not appear fresh. Recipe suggestions are disabled for unsafe or spoiled ingredients."
        )
